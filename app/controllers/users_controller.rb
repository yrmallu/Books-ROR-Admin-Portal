class UsersController < ApplicationController
  ...
  def reset_password
    ...
    @user = User.un_archived.where("lower(username) = lower('#{@a["username"]}')").last unless @a["username"].blank?
  ...
  def set_new_password
    if params[:password] != ""
      ...
      @user = params[:email_id].blank? ? User.un_archived.where("lower(username) = lower('#{params[:username]}')").last : User.un_archived.where("lower(email) = lower('#{params[:email_id]}')").last
