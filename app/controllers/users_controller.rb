...
def user_index
  if params[:query_string] && !(params[:query_string].blank?) 
    ...
    if !@role_id.blank? && params[:school_id].blank?
      @users = User.where(:role_id => @role_id.id).order("created_at DESC").page params[:page]
      set_bread_crumb(@role_id.id)
    elsif !@role_id.blank? && !params[:school_id].blank?
      @users = @school.users.where(:role_id => @role_id.id).order("created_at DESC").page params[:page]
...
if  params[:send_mail].blank?
  ...
  @user.user_email_change_email(current_user.first_name, path, [email_before_save, email_after_save]).deliver if !(email_before_save == email_after_save)
end
else
 get_all_reading_grades
 @assigned_classrooms = @user.classrooms if @user && @user.classrooms
