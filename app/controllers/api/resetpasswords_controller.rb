    ...
    unless user_mail["email"].blank?
      ...
      unless @user.blank?
        ...
        unless user_mail["username"].blank?
          ...
          user_info = {:email => @user.email, :name => User.eql?(@user.class) ?  @user.first_name : @user.name, :full_name =>@user.first_name+" "+@user.last_name.to_s, :username=>student.username, :role => 'Student', :app_type =>'angular', :link => "http://"+request.env['HTTP_HOST']+"/reset_password?password_key="+Base64.encode64(pwd_param.to_s), :url =>  "http://107.21.250.244/books-that-grow-web-app/app_demo_v1.0/#/", :portal_link =>  "http://"+request.env['HTTP_HOST'] } 
        else
          pwd_param = {"email" => @user.email, "a_type" => "angular", "coupon" => coupon}.to_json
          user_info = {:email => @user.email, :name => @user.first_name, :full_name =>@user.first_name+" "+@user.last_name.to_s, :username=>@user.username, :role => @user.role.name, :app_type =>'angular', :link => "http://"+request.env['HTTP_HOST']+"/reset_password?password_key="+Base64.encode64(pwd_param.to_s), :url =>  "http://107.21.250.244/books-that-grow-web-app/app_demo_v1.0/#/", :portal_link =>  "http://"+request.env['HTTP_HOST'] } 
        end
