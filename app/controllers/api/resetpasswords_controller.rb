    ...
    unless user_mail["email"].blank?
      ...
      unless @user.blank?
        ...
        unless user_mail["username"].blank?
          pwd_param = {}
          user_info = {:email => @user.email, :username => User.eql?(@user.class) ?  @user.first_name+" "+@user.last_name.to_s : @user.name, :link => "http://"+request.env['HTTP_HOST']+"/reset_password?password_key="+Base64.encode64(pwd_param), :url =>  "http://"+request.env['HTTP_HOST'] } 
        else
          pwd_param = {"email" => @user.email, "a_type" => "angular"}.to_json
          user_info = {:email => @user.email, :username => @user.first_name+" "+@user.last_name.to_s, :link => "http://"+request.env['HTTP_HOST']+"/reset_password?password_key="+Base64.encode64(pwd_param), :url =>  "http://"+request.env['HTTP_HOST'] } 
        end
