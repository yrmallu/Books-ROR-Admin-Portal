...
android.selection.selectionChanged = function(){
	...
	try{
		...
		var selectionEnd = $("<em id=\"selectionEnd\"/>");
	    var selectionStart = $("<em id=\"selectionStart\"/>");
    	...
    	endRange.insertNode(selectionEnd[0]);
	   	...
	   	window.JsHandler.setContentWidth(document.body.clientWidth);
	   	
	   	// Tell the interface that the selection changed
	   	window.JsHandler.selectionChanged(rangyRange, text, handleBounds, menuBounds);
	}
	catch(err){
		window.JsHandler.jsError(err);
	}
