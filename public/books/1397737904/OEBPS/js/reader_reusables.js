...
(function($, window, document, undefined) {
	...
	TextHighlighter.prototype = {
		...
		bindEvents: function () {
			$("#customhighlightv2").bind("mouseup touchend", {
				self: this
		...
		deserializeHighlights: function(json) {
			...
			var deserializationFn = function (hlDescriptor) {
				...
				{
					timestamp = $highlightObj.attr("class").replace("highlighted note ","");
...
$('.highlighted').live(TWOEVENTEND, function(event) {
	event.stopPropagation();
	var highlightedNode = $(this);
	uniq_timstamp = highlightedNode.attr("class").replace("highlighted ","").replace("note ","");
...
$('#content').on(TWOEVENTEND, '.icon_tinynote', function(event) {
	event.stopPropagation();
...
function fnloadassignedlevel_action()
{
	...
	strUserLevel = objJson.response_data;
	intUserId    = objJson.user_id;
	...
	// if( parseInt($('#delete_cookie').val()) == 1) {
	if(intClearCookies == 1) {
		// Show the story page when downloaded the book for the first time
...
function fngetnotebookmarks_allnew_action()
{
	var objJson = JSON_OPT;
	HIGHLIGHTER.deserializeHighlights(objJson.response_data.note_data);
}
...
function fnGetResponseAction(msg) {
	...
	if (arrResponse.return_code == 0) {
		...
		var strResponseAction = "fn" + strWSMETHOD + "_action()";
		JSON_OPT = objJson;
